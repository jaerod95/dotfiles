...
local filter_table = require("lib.databases.postgres.filter-table")
      ...
      attach_mappings = function(prompt_bufnr)
        actions.select_default:replace(function()
          ...
          if selection and selection[1] then
            ...
            local filter = vim.fn.input("SELECT * FROM " .. table_name .. " WHERE ")
            local sql = interpolate(filter_table, { dbname = database_name, filter = filter, table = table_name })

            vim.api.nvim_buf_set_lines(0, 0, -1, false, split(sql, "\n"))
            local data = execute_database_query(sql, 0, -1)
            if callback then return callback(table_name, data) end
          end
        end)
        return true
      end,
